...
public:
    ...
    /// returns the condition that determines whether an UniverseObject can use this FocusType
...
public:
    ...
    /// returns the condition determining what planets on which this species may spawn
    const Condition::Condition*             Location() const            { return m_location.get(); }
    /// returns the condition for possible targets. may be nullptr if no condition was specified.
    const Condition::Condition*             CombatTargets() const       { return m_combat_targets.get(); }
