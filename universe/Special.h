...
public:
    ...
    /// returns the stealth of the special, which determines how easily it is seen by empires
    const ValueRef::ValueRef<double>*       Stealth() const     { return m_stealth.get(); }
    ...
    /// returns the ValueRef to use to set the initial capacity of the special when placed
    const ValueRef::ValueRef<double>*       InitialCapacity() const { return m_initial_capacity.get(); }
    ...
    /// returns the condition that determines whether an UniverseObject can have this special applied during universe creation
